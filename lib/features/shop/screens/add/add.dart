...
import 'package:stem_shop/common/widgets/product/seller_order_card.dart';
...
void initState() {
  super.initState();
  _loadUserProducts();
  SellerOrdersController.instance.fetchSellerOrders();
}
...
Widget build(BuildContext context) {
  final sellerOrdersController = Get.find<SellerOrdersController>();
  ...
  return Scaffold(
    ...
    body: RefreshIndicator(
      onRefresh: () async {
        await Future.wait([
          _loadUserProducts(),
          sellerOrdersController.fetchSellerOrders(),
        ]);
      },
      child: _isLoading
          ? ListView.separated(
          ...
          : ListView(
              ...
              children: [
                ...
                // ── Divider ───────────────────────────────────────
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: TSizes.spaceBwSections),
                  child: Row(
                    children: [
                ...
                Obx(() {
                  if (sellerOrdersController.isLoading.value) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  if (sellerOrdersController.orders.isEmpty) {
                    return Center(
                      child: Padding(
                  ...
                  return ListView.separated(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: sellerOrdersController.orders.length,
                    separatorBuilder: (_, _) =>
                        const SizedBox(height: TSizes.spaceBwItems),
                    itemBuilder: (_, index) => SellerOrderCard(
                      order: sellerOrdersController.orders[index],
                    ),
                  );
                }),
              ],
