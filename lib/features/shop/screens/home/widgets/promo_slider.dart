...
class TPromoSlider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        CarouselSlider(
          options: CarouselOptions(viewportFraction: 1.06,
          onPageChanged: (index, _) => controller.carouselIndex.value = index),
        
          items: [
            TRoundedImage(
              imagePath: isDark
                  ? TImages.lightDonation
                  : TImages.darkDonation,
            ),
            TRoundedImage(
              imagePath: isDark
                  ? TImages.lightDigital
                  : TImages.darkDigital,
            ),
          ],
        ),
        ...
        Obx(
          () => Row(
            ...
            children: [
              for (int i = 0; i < 2; i++) TCircularContianer(
                width: 20,
                height: 4,
                backgroundColor: controller.carouselIndex.value == i ? TColors.primary : Colors.grey,
